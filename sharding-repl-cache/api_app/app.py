...
async def root():
    ...
    for collection_name in collection_names:
        ...
        pipeline = [
            ...
            {"$group": {"_id": "$shard", "documents_count": {"$sum": "$count"}}}
        ]
        total_count = await collection.count_documents({})
        # Получаем все шардированные коллекции
        pipeline = [
            {
                "$collStats": {
                    "count": {}
        ...
        # Выполняем агрегацию для получения информации по шардированию
        shard_documents = []
        async for shard_stat in collection.aggregate(pipeline):
            shard_documents.append({
                "shard_id": shard_stat["shard"],
                "documents_count": shard_stat["count"]
            })
